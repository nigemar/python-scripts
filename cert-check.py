...
def build_webpage(my_dict):
  ...
  f.write("</head>")
  f.write("<body>")
  f.write("  <h1>Certificate HTML - Produced: %s</h1>" % datetime.now())
  ...
  for key, values in cert_dict.items():
    ...
    f.write('<tr>')
    f.write("<td class=\"%s\">%s</td>" % (status, count))
    f.write("<td class=\"%s\">%s</td>" % (status, values['filename']))
    f.write("<td class=\"%s\">%s</td>" % (status, values['subject']))
    f.write("<td class=\"%s\">%s</td>" % (status, int(values['days_remaining'])))
    f.write("<td class=\"%s\">%s</td>" % (status, values['end_date']))
    f.write("<td class=\"%s\">%s</td>" % (status, values['thumbprint']))
    #f.write("serial:      %s" % values['serial'])
    #f.write("status:      %s" % values['status'])
    #f.write("sans:          ")
    #sans = values['sans'][0]
    #sans = sans.split(',')
    #san_count = 1
    #for san in sans:
    #  f.write("    %s. %s" % (san_count,san.replace('DNS:', '')))
    #  san_count+=1
    f.write("</tr>")
    count+=1
